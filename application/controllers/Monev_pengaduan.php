    ...
    public function index()
{
    ...
    // ✅ TAMBAHKAN: Generate date range dengan waktu
    $date_range = $this->get_date_range_with_time($periode, $tahun);
    
    // Top Cards Data - SEKARANG PAKAI $date_range
    $data['total_permohonan'] = $this->M_monev_pengaduan->get_total_permohonan($date_range);
    $data['dalam_proses'] = $this->M_monev_pengaduan->get_dalam_proses($date_range);
    $data['dipenuhi'] = $this->M_monev_pengaduan->get_dipenuhi($date_range);
    $data['telah_disampaikan'] = $this->M_monev_pengaduan->get_telah_disampaikan($date_range);
    
    // Status Permohonan (untuk grafik donut pertama)
    $data['status_permohonan'] = $this->M_monev_pengaduan->get_status_permohonan($date_range);
    
    // Via Permohonan (tabel)
    $data['via_permohonan'] = $this->M_monev_pengaduan->get_via_permohonan($date_range);
    
    // Status Pemohon (untuk grafik donut kedua)
    $data['status_pengirim'] = $this->M_monev_pengaduan->get_status_pengirim($date_range);
...
/**
 * Fungsi tambahan untuk generate date range DENGAN WAKTU
 */
...
public function get_detail_via()
{
    ...
    try {
        ...
        $jenis_periode = $this->input->get('jenis_periode') ?: 'semua';
        $periode = $this->input->get('periode');
        ...
        // ✅✅✅ PERBAIKI: PAKAI DATE RANGE, BUKAN FILTER
        $date_range = $this->get_date_range_with_time($periode, $tahun);
        
        // Ambil detail via dari model
        $detail = $this->M_monev_pengaduan->get_detail_via($via_index, $date_range);
...
public function get_detail_status_permohonan()
{
    ...
    try {
        ...
        $detail = $this->M_monev_pengaduan->get_detail_status_permohonan($status_key, $date_range);
...
public function get_detail_status_pengirim()
{
    ...
    try {
        ...
        $detail = $this->M_monev_pengaduan->get_detail_status_pengirim($status_index, $date_range);
